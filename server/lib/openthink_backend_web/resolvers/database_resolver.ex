defmodule OpenthinkBackendWeb.DatabaseResolver do
  ...
  def reorder_view_entries(_root, %{input: %{view_id: view_id, entry_id: entry_id, index: index}}, %{context: %{user_id: _user_id}}) do
  ...
  def reorder_view_groups(_root, %{input: %{view_id: view_id, group_id: group_id, index: index}}, %{context: %{user_id: _user_id}}) do
    case from(v in View, where: v.view_id == ^view_id)
  ...
  def remove_entry(_root, %{input: %{view_id: view_id, entry_id: entry_id}}, %{context: %{user_id: _user_id}}) do
    {num, val} = from(v in View, where: v.view_id == ^view_id)
  ...
  def remove_group(_root, %{input: %{view_id: view_id, group_id: group_id}}, %{context: %{user_id: _user_id}}) do
