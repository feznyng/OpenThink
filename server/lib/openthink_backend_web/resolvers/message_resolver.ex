defmodule OpenthinkBackendWeb.MessageResolver do
  ...
  alias OpenthinkBackend.{Message, RoomUser, SpaceUser}
  import Ecto.Query
  ...
  def get_message_by_id(_root, %{message_id: message_id}, _res) do
    msg =
      from(m in MessageQueries.get_messages(),
  ...
  def get_room_by_id(_root, %{room_id: id}, info) do
    ...
    if is_nil(room) do
      ...
      # this is a room whose name may need to be manually computed based on the users within it
      room =
        if is_nil(room.space_id) and is_nil(room.name) do
          users =
            Repo.all(
          ...
          Map.merge(
            room,
            OpenthinkBackendWeb.RoomUtils.generate_room_name_and_icon(
              room,
              users,
  ...
  def get_rooms_by_user_ids(_root, args, %{context: %{user_id: user_id}}) do
    ...
    room =
      if !is_nil(room) do
        ...
        Map.merge(
          room,
          OpenthinkBackendWeb.RoomUtils.generate_room_name_and_icon(room, room.users, user_id)
        )
      else
        room
      end
  ...
  def send_message(_parent, %{input: input}, %{context: %{user_id: user_id}}) do
    {:ok, message} =
      OpenthinkBackendWeb.MessageQueries.create_message(Map.put(input, :created_by, user_id))

    node =
      if !!Map.get(message, :room_id) do
        message
  ...
  def create_room(_parent, %{input: input}, %{context: %{user_id: user_id}}) do
    {:ok, room} =
      OpenthinkBackendWeb.MessageQueries.create_room(
        Map.put(input, :created_by, user_id),
        user_id
      )
    ...
    room =
      ...
      |> Map.merge(
        OpenthinkBackendWeb.RoomUtils.generate_room_name_and_icon(room, users, user_id)
      )

    # room is a dm so let each user except for the creator know it exists
    if is_nil(room.space_id) do
      users
      |> Enum.each(fn u ->
        Absinthe.Subscription.publish(OpenthinkBackendWeb.Endpoint, %{room_edge: %{node: room}},
          direct_messages: "user:#{u.user_id}"
        )
      end)
    end
    ...
    {:ok,
     %{
       # if this is a channel this will ensure the space's channel list is updated
  ...
  def update_last_room_id(_parent, %{input: input}, %{context: %{user_id: user_id}}) do
    {:ok, user} =
      OpenthinkBackendWeb.MessageQueries.set_last_room_id(
        input.room_id,
        user_id,
        !is_nil(input.space_id)
      )
    ...
    if Map.has_key?(message, :room_id) and Map.has_key?(message, :space_id) do
      ...
      OpenthinkBackendWeb.Endpoint.broadcast(
        ...
        %{
          ...
          space_graphql_id: space_graphql_id
        }
      )

      # update the database
      OpenthinkBackendWeb.MessageQueries.set_messages_notifications(message, topic)
    end

    {:ok, "Worked"}
  end

  def reorder_space_channels(
        _root,
        %{
          input: %{
            room_id: room_id,
