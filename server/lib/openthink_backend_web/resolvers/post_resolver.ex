defmodule OpenthinkBackendWeb.PostResolver do
  ...
  def delete_post(_parent, %{input: input}, %{context: %{user_id: _user_id}}) do
    case PostQueries.delete_post(input.post_id) do
      {:error, err} -> {:error, err}
      {:ok, {:ok, post}} -> {:ok, %{deleted_post_id: Node.to_global_id("Post", post.post_id), deleted_post: post}}
      _ -> {:error, "Was unable to update"}
    end
  end
  ...
  def batch_delete_post(_parent, %{input: input}, %{context: %{user_id: _user_id}}) do
    ...
    case PostQueries.batch_delete_post(input.post_ids) do
      {:ok, posts} -> {:ok, %{deleted_post_ids: Enum.map(posts, fn post -> Node.to_global_id("Post", post.post_id) end)}}
      val -> val
    end
  end

  def reorder_space_posts_of_type(_root, %{input: %{post_id: post_id, type: type, space_id: space_id, old_index: old_index, new_index: new_index}}, %{context: %{user_id: _user_id}}) do
    PostQueries.reorder_space_post_of_type(post_id, space_id, type, old_index, new_index)
  end

  def reorder_related_posts_of_type(_root, %{input: %{space_id: space_id, relation_id: relation_id, parent_post_id: post_id, type: type, old_index: old_index, new_index: new_index}}, %{context: %{user_id: _user_id}}) do
    PostQueries.reorder_related_post_of_type(space_id, post_id, relation_id, type, old_index, new_index)
  end

  def get_post_spaces(pagination_args, %{source: %{post_id: post_id}, context: %{user_id: user_id}}) do
    Absinthe.Resolution.Helpers.batch(
  ...
  def get_space_referenced(post, _args, %{source: %{attributes: attributes}}) do
    Absinthe.Resolution.Helpers.batch(
  ...
  def change_post_vote(_root, %{input: %{post_id: post_id, space_id: space_id, upvote: upvote}}, %{context: %{user_id: user_id}}) do
  ...
  def delete_post_vote(_root, %{input: %{post_vote_id: post_vote_id}}, %{context: %{user_id: _user_id}}) do
    case PostQueries.delete_post_vote(post_vote_id) do
      {:ok, deleted_vote} ->

        {:ok, %{my_vote: Map.put(deleted_vote, :post_vote_id, nil), post: get_updated_votes_post(deleted_vote.post_id)}}
      error -> error
    end
  end
  ...
  def add_reaction(_root, %{input: %{name: name, space_id: space_id, emoji: emoji, post_id: post_id}}, %{context: %{user_id: user_id}}) do
  ...
  def add_reaction(_root, %{input: %{name: name, space_id: space_id, emoji: emoji, message_id: message_id}}, %{context: %{user_id: user_id}}) do
  ...
  def delete_reaction(_root, %{input: %{reaction_id: reaction_id}}, %{context: %{user_id: user_id}}) do
    case PostQueries.delete_reaction(reaction_id) do
      {:ok, deleted_reaction} ->
        reaction = if !!Map.get(deleted_reaction, :message_id) do
        ...
        case reaction do
          nil -> {:ok, %{deleted_reaction_id: Node.to_global_id("Reaction", ReactionTypes.reaction_id_fetcher(deleted_reaction, nil))}}
          reaction ->
            {:ok, %{reaction: reaction}}
        end
      error -> error
  ...
  def move_post(_root, %{input: input}, %{context: %{user_id: user_id}}) do
    ...
    if is_nil(parent_post_id) do
      ...
      %{prev_parent_post_id: prev_parent_post_id} = Repo.transaction(fn ->
        {:ok, deleted_relation} = PostQueries.delete_relation(relation_id)
        Repo.one(from sp in SpacePost, where: sp.post_id == ^post_id and sp.space_id == ^space_id)
      ...
      {:ok, %{deleted_post_id: Node.to_global_id("Post", post_id), prev_parent_post_id: Node.to_global_id("Post", prev_parent_post_id)}}
    else
      # moving to a post from either a space or another post
      case Repo.transaction(fn ->
        deleted_relation = if !!relation_id do
          {:ok, relation} = PostQueries.delete_relation(relation_id)
          relation
        end
        {:ok, created_relation} = PostQueries.create_relation(parent_post_id, post_id, user_id, space_id, view_id: view_id, index: index)
        if !!space_id do
          ...
          if !deleted_relation or space_post.parent_post_id == deleted_relation.post1_id do
            parent_space_post = Repo.one(
              from sp in SpacePost,
              where: sp.post_id == ^created_relation.post1_id and sp.space_id == ^space_id,
              limit: 1
            )
            ...
            if !!parent_space_post do
              ...
              space_post
              |> SpacePost.update_path_changeset(%{path: path, parent_post_id: parent_space_post.post_id})
              |> Repo.update
              path = if String.length(path) > 1 do path <> "." else path end
        ...
        {:ok, %{created_relation: relation, deleted_relation: deleted_relation}} ->
          ...
          obj = if !!deleted_relation do
            obj
            |> Map.put(:prev_parent_post_id, Node.to_global_id("Post", Map.get(deleted_relation, :post1_id)))
          else
            obj
          end
          {:ok, obj}
  ...
  def create_relation(_root, %{input: %{post_id: post_id, parent_post_id: parent_post_id, space_id: space_id, view_id: view_id, index: index}}, %{context: %{user_id: user_id}}) do
    case PostQueries.create_relation(parent_post_id, post_id, user_id, space_id, view_id: view_id, index: index) do
      {:ok, relation} ->
        {:ok, %{
          post_edge: %{
  ...
  def create_link(_root, %{input: %{post1_title: post1, post2_title: post2, space_id: space_id}}, %{context: %{user_id: user_id}}) do
    parent_post_id = from(
      p in Post,
      select: p.post_id,
  ...
  def delete_relation(_root, %{input: %{relation_id: relation_id}}, %{context: %{user_id: _user_id}}) do
    case PostQueries.delete_relation(relation_id) do
      {:ok, _relation} -> {:ok, %{deleted_relation_id: Node.to_global_id("PostRelation", relation_id)}}
      val -> val
    end
  end

  def vote_poll(_root, %{input: %{post_id: post_id, attribute_id: aid, attribute_type: type, selected: option}}, %{context: %{user_id: user_id}}) do
  ...
  def import_post_file(_root, %{input: %{post_id: post_id, space_id: space_id, file: file}}, %{context: %{user_id: user_id}}) do
    PostQueries.import_post_file(user_id, file, space_id, post_id)
  end

  def import_post_file(_root, %{input: %{space_id: space_id, file: file}}, %{context: %{user_id: user_id}}) do
  ...
  def get_space_post(pagination_args, %{source: post}) do
    cond do
      Map.has_key?(post, :space_post) && (!Map.get(pagination_args, :space_id) || Map.get(post, :space_post) |> Map.get(:space_id) == pagination_args.space_id) ->
        {:ok, post.space_post}
  ...
  def get_space_user(args, %{source: post}) do
    cond do
      Map.has_key?(post, :space_author) && (!Map.get(args, :space_id) || Map.get(post, :space_author) |> Map.get(:space_id) == args.space_id) ->
