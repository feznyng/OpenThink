defmodule OpenthinkBackendWeb.UserQueries do
  ...
  require Logger
  import Ecto.Query
  alias OpenthinkBackend.{User, Connection, SpaceUser, UserSkill, UserTopic, InviteLink, Favorite, PostUser, SpacePost, Space, Post, Room, RoomUser}
  ...
  def get_connections(user_id, _curr_user, params) do
    from(u in User,
    inner_join: c in Connection, on: ((u.user_id == c.user1_id and c.user2_id == ^user_id) or (u.user_id == c.user2_id and c.user1_id == ^user_id)),
  ...
  defp exclude_requested(query, params) do
    if Map.has_key?(params, :exclude_requested) and params.exclude_requested do
      from u in query,
      inner_join: c in Connection, on: (u.user_id == c.user1_id) or (u.user_id == c.user2_id),
      where: c.accepted
    else
      query
    end
  ...
  def generate_unread_vals(room_users) do
    Enum.reduce(
      ...
      fn ru, acc ->
        acc
        |> Map.put(:unread_messages, Map.get(acc, :unread_messages) or (!is_nil(ru.unread) and ru.unread))
        |> Map.put(:unread_direct_messages, Map.get(acc, :unread_direct_messages) or (!is_nil(ru.unread) and ru.unread and is_nil(ru.room.space_id)))
      end
    )
  end
  ...
  def get_user_rooms(user_id) do
    Repo.all(
      from u in "users",
      left_join: ru in "room_users", on: ru.user_id == u.user_id,
      left_join: r in "rooms", on: r.room_id == ru.room_id,
  ...
  def get_user_posts(user_id, opts \\ %{}) do
    ...
    space_ids = from(su in SpaceUser, select: su.space_id, where: su.user_id == ^user_id) |> Repo.all
  ...
  def include_assigned(query, user_id, %{include_assigned: true}) do
    from p in query,
    where: p.created_by == ^user_id or p.post_id in subquery(
      from pu in PostUser,
      select: pu.post_id,
  ...
  def get_user_direct_messages(user_id) do
    ...
    where: ru.user_id == ^user_id and (is_nil(ru.archived) or not ru.archived) and is_nil(r.space_id),
  ...
  def temp_sign_in(name, true) do
    ...
    lastname = if firstname == lastname do
      ""
    else
      lastname
    end
  ...
  def create_connection(user_id, connection) do
    if connection.user1_id != user_id and connection.user2_id != user_id do
      ...
      other_user_id = if connection.user1_id != user_id do connection.user1_id else connection.user2_id end
      case Connection.changeset(%Connection{}, connection) |> Repo.insert do
        {:ok, connection} ->
          NotificationQueries.spawn_notification_generation([other_user_id], %{type: "connection request"}, user_id)
          {:ok, connection}
        val -> val
      end
    end
  ...
  def accept_connection(update_connection, user_id) do
    ...
    |> Repo.update do
      {:ok, connection} ->
        other_user_id = if connection.user1_id != user_id do connection.user1_id else connection.user2_id end
        NotificationQueries.spawn_notification_generation([other_user_id], %{type: "connection accept"}, user_id)
        {:ok, connection}
      val -> val
    end
  end
  ...
  def delete_connection(other_user_id, user_id) do
    ...
    where: (c.user1_id == ^other_user_id and c.user2_id == ^user_id) or (c.user2_id == ^other_user_id and c.user1_id == ^user_id))
    |> Repo.one
    |> Repo.delete
  end
  ...
  def get_users_connection(curr_user_id, user_ids) do
    connections = Repo.all(
      from c in Connection,
      where: (c.user1_id == ^curr_user_id and c.user2_id in ^user_ids) or (c.user2_id == ^curr_user_id and c.user1_id in ^user_ids)
    )
    Map.new(user_ids, fn id -> {id, Enum.find(connections, fn sp -> (sp.user2_id == id and sp.user1_id == curr_user_id) or (sp.user1_id == id and sp.user2_id == curr_user_id) end)} end)
  end
  ...
  def get_feed(user_id, opts \\ %{}) do
    ...
    where: (is_nil(s.archived) or s.archived == false) and su.user_id == ^user_id and p.type in ^PostQueries.get_user_facing_post_types() and (sp.deleted == false or is_nil(sp.deleted)))
    |> PostQueries.sort_posts_query(opts)
