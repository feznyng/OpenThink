...
import { useFragment, useLazyLoadQuery, usePaginationFragment, usePreloadedQuery } from 'react-relay';
    ...
    const {data, ...args} = usePaginationFragment(
        graphql`
            ...
            @refetchable(queryName: "FeedPaginationQuery") {
                feed(first: $feedCount, after: $feedCursor, sortBy: $sortBy) @connection(key: "FeedFragment_feed") {
                    __id
                    edges {
                        node {
                            postId
                ...
                <React.Fragment>
                    {
                        ...
                        <Typography variant="h5" style={{textAlign: 'center'}}>
                            Hmm, there's nothing in your feed. Join a <Link onClick={() => history.push('/spaces')}>group</Link> to get started!
                        </Typography>
                        :
                        <Typography variant="h5" style={{textAlign: 'center'}}>
