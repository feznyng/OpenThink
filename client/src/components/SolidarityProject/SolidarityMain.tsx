...
export default function SolidarityMain({
  ...
  const attributes = post?.attributes?.edges
    ? post.attributes.edges
        ...
        .filter(
          ({ name }) =>
            !["Coalition", "Space", "icons", "Active Membership Size"].includes(
              name,
            ),
        )
    : [];

  const { postID, spacePage, spaceID, subPage, viewId } =
    useParams<SpaceViewParams>();
  ...
  const { nodes, links, nodeMap, linkMap, inLinkMap, additionalAttrs } =
    useMemo(() => {
      let nodes: Node[] = [];
      let links: Edge[] = [];
      ...
      if (postGraph && postGraph.nodes!!.length > 0) {
        links = postGraph.edges
          ? postGraph.edges.map(({ post1Id, post2Id }: any) => {
        ...
        const coalitionAttrId = post?.attributes?.edges?.find(
          (e) => e?.node?.name === "Coalition",
        )?.node?.attributeId;

        nodes = postGraph.nodes
          ? postGraph.nodes.map((post: any) => {
              const node: Node = postToNode(post as any, { omitImage: true });
              const coalitionAttr = post?.attributeValues?.find(
                (attr: any) => attr.attributeId === coalitionAttrId,
              );

              const isCoalition =
                coalitionAttr?.textValue === "TRUE" ||
                post.type === "Coalition";
  ...
  const [postViewQueryRef, loadPostViewQuery] = useQueryLoader(
  ...
  const onNodeClick = ({ id, x, y }: Node) => {
    history.replace(
      `/space/${spaceID}/${spacePage}/${id}` + history.location.search,
    );
  };
  ...
  const toggleAttribute = (name: string) => {
    ...
    const filterOutInactive =
      inactiveAttr && newFilters.includes(inactiveAttr.id!!);
    ...
    let filteredNodes = nodes.filter(({ attributes, type, id }) => {
      if (nodeBlacklist.has(id)) return false;
      ...
      for (let i = 0; i < attributes.length; i++) {
        ...
        if (
          filterOutInactive &&
          inactiveAttr &&
          currValue.attributeId == inactiveAttr.id &&
          currValue.value
        ) {
          return false;
        }
    ...
    const filteredLinks = links.filter(({ source, target }) => {
      return (
        acceptableIds.has((source as any).id) &&
        acceptableIds.has((target as any).id)
      );
    });
  ...
  return (
    <PostDrawerLayout
      ...
      main={
        <div style={{ position: "relative", height: "100%", width: "100%" }}>
          {nodes.length > 0 ? (
            ...
            >
              <Typography variant="h6">This map is empty</Typography>
            </div>
          )}
          {canImport && (
            <GraphActions
              onCreateNode={refresh}
              onCreateEdge={refresh}
              onImport={refresh}
              space={space}
      ...
      postView={
        <SuspenseLoader queryRef={postViewQueryRef} fallback={<div />}>
          <Paper
            style={{
              height: "100%",
              overflow: "auto",
