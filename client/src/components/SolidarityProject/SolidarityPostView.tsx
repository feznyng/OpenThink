...
export default function SolidarityPostView({
  queryRef,
  style,
  onClose,
  onChange,
}: SolidarityPostViewProps) {
  const { post, me, space } = usePreloadedQuery<SolidarityPostViewQuery>(
    graphql`
      query SolidarityPostViewQuery(
        $spaceId: Int!
        $postId: ID!
        $userCount: Int!
        $userCursor: String
  ...
  const { data } = usePaginationFragment<
    SolidarityPostViewPaginationQuery,
    SolidarityPostViewFragment$key
  >(
    graphql`
  ...
  const tasks = data?.posts?.edges
    ? data?.posts?.edges.map(({ node }: any) => node)
    : [];
  ...
  const [commitComplete] = useMutation<SolidarityPostViewCompleteMutation>(
    graphql`
      mutation SolidarityPostViewCompleteMutation(
    ...
    commiteCreateTempAccount({
      ...
      onCompleted: (response) => {
        ...
        if (user) {
          ...
          state.postId &&
            state.connectionId &&
            complete(state.postId, state.connectionId, user.userId);
          setState({
            ...state,
            postId: null,
            name: "",
  ...
  const [commiteCreateTempAccount] =
    useMutation<SolidarityPostViewAccountMutation>(graphql`
  ...
  const [commitUpdatePost] = useMutation(graphql`
    ...
    commitUpdatePost({
      variables: {
        input: {
          ...
          ...post,
          delta: JSON.stringify(post.delta),
        },
      },
    });
    onChange(post);
      ...
      </div>
      {
        <Fragment>
          ...
          currentPost.postId.toString() ===
            queryRef.variables.postId.toString() ? (
            <PostView
              ...
              onContentChange={(delta) =>
                onPostChange({ ...currentPost, delta })
              }
          ...
          <div>
            <GeneralTabs
              ...
              onClick={(link) =>
                history.replace(
                  location.pathname + "?" + changeQueryString("tab", link),
                )
              }
            />
