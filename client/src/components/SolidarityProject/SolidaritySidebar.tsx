...
export default function SolidaritySidebar({ project }: SolidaritySidebarProps) {
  const { data, loadNext, hasNext } = usePaginationFragment<
    SolidaritySidebarQuery,
    SolidaritySidebarFragment$key
  >(
    graphql`
      fragment SolidaritySidebarFragment on Space
      @refetchable(queryName: "SolidaritySidebarQuery") {
        spaceId
        posts(first: $mapCount, after: $mapCursor, filterTypes: ["Database"])
          @connection(key: "SolidaritySidebarFragment_posts") {
          __id
          edges {
            node {
              postId
  ...
  const [commitCreateMap] = useMutation<SolidaritySidebarCreateMutation>(
    graphql`
      mutation SolidaritySidebarCreateMutation(
        $input: PostInput!
        $connections: [ID!]!
      ) {
  ...
  const [commitCreateView] = useMutation<SolidaritySidebarCreateViewMutation>(
    graphql`
      mutation SolidaritySidebarCreateViewMutation(
  ...
  const [commitDeleteMap] = useMutation<SolidaritySidebarDeleteMutation>(
    graphql`
      mutation SolidaritySidebarDeleteMutation(
  ...
  const deleteMap = () => {
    ...
    commitDeleteMap({
      ...
      onCompleted: () =>
        data.posts?.edges && data.posts.edges.length > 0
          ? history.replace(
              `/space/${spaceID}/${data.posts.edges[0]!!.node?.postId}`,
            )
      ...
      >
        {data?.posts?.edges?.map((e) => (
          <PostListItem
            ...
            post={e!!.node}
            onClick={(postId) =>
              history.replace(`/space/${data.spaceId}/${postId}`)
            }
            menuOptions={
              canAddMap
                ? [
