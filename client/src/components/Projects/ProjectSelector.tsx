...
export default function ProjectSelector({ queryRef, style }: ProjectSelectorProps) {
    const { space, currentProject } = usePreloadedQuery<ProjectSelectorQuery>(
        graphql`
            query ProjectSelectorQuery($spaceId: Int, $parentSpaceId: Int, $inGroup: Boolean!, $spaceCount: Int!, $spaceCursor: String) {
    ...
    const { data } = usePaginationFragment<ProjectSelectorPaginationQuery, ProjectSelectorFragment$key>(
        graphql`
            fragment ProjectSelectorFragment on Space @refetchable(queryName: "ProjectSelectorPaginationQuery") {
                spaces(first: $spaceCount, after: $spaceCursor, filters: {project: true}) @connection(key: "ProjectSelectorFragment_spaces") {
                    edges {
                        node {
                            spaceId
                            ...SpacePreviewFragment 
