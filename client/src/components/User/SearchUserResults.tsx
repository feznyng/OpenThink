...
export default function SearchUserResults({space, query, ...props}: SearchUserResultsProps & Partial<UserListProps>,) {
    ...
    const data = useLazyLoadQuery<SearchUserResultsQuery>(
        graphql`
            query SearchUserResultsQuery($query: String!, $searchCount: Int!, $searchCursor: String, $spaceId: Int) {
    ...
    const usersResult = usePaginationFragment<SearchUserResultsPaginationQuery, SearchUserResultsFragment_users$key>(
        graphql`
            fragment SearchUserResultsFragment_users on Query 
            @refetchable(queryName: "SearchUserResultsPaginationQuery") {
                searchUsers(excludeSpaceId: $spaceId, first: $searchCount, after: $searchCursor, query: $query) @connection(key: "onnectionsSearchResultsFragment_searchUsers") {
                    edges {
                        node {
                            userId
                        }
