...
export default function GroupSelectorResults({
  root,
  style,
  onClose,
  creatingGroup,
  query,
}: GroupSelectorResultsProps) {
  const { data, refetch } = usePaginationFragment<
    GroupSelectorResultsQuery,
    GroupSelectorResults$key
  >(
    graphql`
      fragment GroupSelectorResults on RootQueryType
      @refetchable(queryName: "GroupSelectorResultsQuery") {
        spaces(
          first: $spaceCount
          after: $spaceCursor
          filters: { query: $query }
        ) @connection(key: "GroupSelectorResults_spaces") {
          __id
          edges {
            node {
              spaceId
      ...
      <SpaceList
        ...
        spaceListItemProps={{
          onClick: (spaceId: number) =>
            history.replace(`/get-started/spaces/${spaceId}`),
        }}
        style={style}
      />
      <SpaceCreator
