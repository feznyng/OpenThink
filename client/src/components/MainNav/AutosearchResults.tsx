...
import { useLazyLoadQuery, usePaginationFragment, usePreloadedQuery } from 'react-relay'
...
export default function AutosearchResults({query, closeResults}: AutosearchResultsProps) {
    const results = useLazyLoadQuery<AutosearchResultsQuery>(
        graphql`
            query AutosearchResultsQuery($query: String!, $groupNum: Int!, $groupCursor: String) {
    ...
    const {data} = usePaginationFragment<AutosearchGroupsResultsQuery, AutosearchResults_groups$key>(
        graphql`
            fragment AutosearchResults_groups on Query 
            @refetchable(queryName: "AutosearchGroupsResultsQuery") {
                searchGroups: spaces(first: $groupNum, after: $groupCursor, filters: {query: $query, excludeChildren: true, project: false}) @connection(key: "AutosearchResults_searchGroups") {
                    __id
                    edges {
                        node {
                            spaceId
        ...
        function arrowKeyHandler(e: KeyboardEvent) {
            switch (e.key) {
                ...
                case 'Enter':
                    ...
                    e.stopPropagation()
                    if (selected != null) {
                        console.log('here', `/space/${spaces!![selected]!!.node!!.spaceId}`)
                        spaces && history.push(`/space/${spaces[selected]!!.node!!.spaceId}`)
                        closeResults()
                    }
