...
import { AppBar, Fab, Toolbar, BottomNavigation, BottomNavigationAction, Button, IconButton, Menu, Badge, Fade, Tooltip, SwipeableDrawer, BadgeProps, CircularProgress } from '@material-ui/core';
import graphql from 'babel-plugin-relay/macro';
import { KeyboardArrowUp, AccountCircle, Notifications, Chat, Menu as MenuIcon, Home } from '@material-ui/icons';
  ...
  return (
    <div style={{height: menuHeight}}>
      ...
      <AppBar 
        ...
        style={{ backgroundColor: 'transparent', padding: 0, position: (!signedIn && currLocation === 'dashboard') ? 'absolute' : 'fixed'}}
      >
        <Toolbar className={classes.toolbar} style={{ backgroundColor: darkMode ? paperColor : 'white', height: 55}} variant="dense">
          {
            me?.userId && 
          ...
          <Link to="/" style={{textDecoration: 'none'}}>
            <img
                alt=""
                style={{height: 40, cursor: 'pointer', marginLeft: -10, marginBottom: -3}}
            ...
            <div className={classes.sectionDesktop}>
              {
                options.map(({title, link, location, activeIcon, inactiveIcon}) => {
                  let badgeProps: BadgeProps = {variant: 'standard', color: 'error', invisible: true}
                  if (location === 'messages') {
                    badgeProps.invisible = !me?.unreadMessages
                  } else if (location === 'notifications') {
                    
                    const numNotifications = me?.notificationsNum ? me?.notificationsNum : 0
                  ...
                  return (
                    ...
                    >
                      <Button 
                        ...
                        style={{color: (currLocation === location ? '#2196f3' : ''), textDecoration: 'none', textTransform: 'none', marginLeft: 10}} 
                      >
                        <div>
                          <Badge 
                            {...badgeProps}
                        ...
                        </div>
                        {
                          ...
                          <Fade in={true} timeout={300}>
                            <div style={{width: '100%', borderStartStartRadius: 25, borderStartEndRadius: 25, position: 'absolute',  bottom: -2, backgroundColor: '#2196f3', height: 5}}/>
                          </Fade>
                        }
                      </Button>
                    </Tooltip>
      ...
      </AppBar>
      {
        ...
        <div>
          ...
          >
            <BottomNavigationAction onClick={() => history.replace('/')} icon={<Home/>} />
            <BottomNavigationAction onClick={() => history.replace('/spaces')} icon={<Groups/>} />
           
            <BottomNavigationAction onClick={() => history.replace('/notifications')} icon={<Notifications/>} />
          </BottomNavigation>
        </div>
      }
      {
