      ...
      <ForceGraph
        ...
        onNodeClick={(node, event) => {
          ...
          if (
            prevClick &&
            prevClick.node === node &&
            (now as any) - prevClick.time < DBL_CLICK_TIMEOUT
          ) {
            setPrevClick(null);
        ...
        nodeCanvasObject={(node, ctx) => {
          ...
          if (drawingAttrs) {
            if (attrRenderingMethod === "circle") {
              ...
              const num = nd.attributes.filter(
                (at: AttributeValue) => !!attributeMap.get(at.attributeId),
              ).length;
              let start = 0;
        ...
        nodeRelSize={
          NODE_R + (attrRenderingMethod === "circle" ? lineWidth : 0)
        }
        linkColor={(l) =>
          (l as Edge).color ? (l as Edge).color : (linkColor as any)
        }
