...
export default function UserSearchResults({
  ...
  const { data, hasNext, loadNext } = usePaginationFragment<
    ...
    UserSearchResultsFragment$key
  >(
    graphql`
      fragment UserSearchResultsFragment on RootQueryType
      @refetchable(queryName: "UserSearchResultsQuery") {
        searchUsers(query: $query, first: $userCount, after: $userCursor)
          @connection(key: "UserSearchResultsFragment_searchUsers") {
          edges {
            node {
  ...
  const users = data.searchUsers?.edges
    ? data.searchUsers.edges.map((e) => <UserListItem user={e!!.node} />)
    : [];
